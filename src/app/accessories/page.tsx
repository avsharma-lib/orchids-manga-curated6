...
export default function AccessoriesPage() {
  const { allAccessories } = useProducts();
  return (
              ...
              {allAccessories.map((accessory, index) => (
                <motion.div key={accessory.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03, duration: 0.3 }}>
                  <Link href={`/accessorys/${accessory.id}`} className="group block">
                    ...
                    <h3 className="text-sm font-medium text-[var(--ink)] mb-1 group-hover:text-[var(--crimson)] transition-colors line-clamp-2">{accessory.title}</h3>
                    {accessory.series && <p className="text-xs text-[var(--stone)] mb-2">{accessory.series}</p>}
                    <div className="flex items-baseline gap-2">
                      <span className="text-sm font-medium text-[var(--ink)]">{formatPrice(accessory.price)}</span>
