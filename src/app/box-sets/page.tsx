...
export default function BoxSetsPage() {
  const { allBoxSets, allManga } = useProducts();
  const availableBoxSets = allBoxSets.filter(bs => allManga.some(m => m.id === bs.mangaId) || !bs.mangaId);

  return (
    <div className="bg-[var(--paper)] pt-20 min-h-screen">
      <section className="py-16 border-b border-[var(--mist)]">
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <nav className="flex items-center gap-2 text-sm text-[var(--stone)] mb-6">
            <Link href="/" className="hover:text-[var(--ink)]">Home</Link><span>/</span><span className="text-[var(--ink)]">Box Sets</span>
          </nav>
          <h1 className="text-4xl md:text-5xl text-[var(--ink)]" style={{ fontFamily: 'var(--font-heading)' }}>All Box Sets</h1>
