            ...
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="relative">
              <div className="sticky top-32">
                <div className="relative aspect-square bg-[var(--mist)] overflow-hidden rounded-lg">
                  {imgError ? <div className="absolute inset-0 flex items-center justify-center"><span className="text-xl text-[var(--stone)]">{hoodie.title}</span></div> : <Image src={hoodie.image} alt={hoodie.title} fill className="object-cover" sizes="50vw" priority onError={() => setImgError(true)} unoptimized />}
                </div>
                <div className="absolute top-4 left-4"><span className="px-3 py-1.5 text-xs tracking-widest uppercase bg-[var(--ink)] text-white">Hoodie</span></div>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="lg:py-8">
              {hoodie.brand && <p className="text-sm tracking-wider text-[var(--stone)] mb-2">{hoodie.brand}</p>}
              <h1 className="text-4xl md:text-5xl text-[var(--ink)] mb-6" style={{ fontFamily: 'var(--font-heading)' }}>{hoodie.title}</h1>
              <div className="flex items-baseline gap-4 mb-8">
                <span className="text-3xl font-medium text-[var(--ink)]">{formatPrice(hoodie.price)}</span>
                <span className="text-lg text-[var(--stone)] line-through">{formatPrice(hoodie.originalPrice)}</span>
                <span className="px-2 py-1 text-xs tracking-wider bg-[var(--crimson)] text-white">SAVE {Math.round((1 - hoodie.price / hoodie.originalPrice) * 100)}%</span>
              </div>
              {hoodie.description && <p className="text-lg text-[var(--stone)] leading-relaxed mb-8">{hoodie.description}</p>}
              <div className="border-t border-b border-[var(--mist)] py-6 mb-8 space-y-3">
                {hoodie.characterName && <div className="flex justify-between text-sm"><span className="text-[var(--stone)]">Collection</span><span className="text-[var(--ink)]">{hoodie.characterName}</span></div>}
                {hoodie.series && <div className="flex justify-between text-sm"><span className="text-[var(--stone)]">Series</span><span className="text-[var(--ink)]">{hoodie.series}</span></div>}
                {hoodie.material && <div className="flex justify-between text-sm"><span className="text-[var(--stone)]">Material</span><span className="text-[var(--ink)]">{hoodie.material}</span></div>}
                {hoodie.height && <div className="flex justify-between text-sm"><span className="text-[var(--stone)]">Height</span><span className="text-[var(--ink)]">{hoodie.height}</span></div>}
                {hoodie.weight && <div className="flex justify-between text-sm"><span className="text-[var(--stone)]">Weight</span><span className="text-[var(--ink)]">{hoodie.weight}</span></div>}
              </div>
