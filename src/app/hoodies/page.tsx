...
export default function ActionFiguresPage() {
  const { allHoodies } = useProducts();
  return (
    <div className="bg-[var(--paper)] pt-20 min-h-screen">
      <section className="py-16 border-b border-[var(--mist)]">
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <nav className="flex items-center gap-2 text-sm text-[var(--stone)] mb-6"><Link href="/" className="hover:text-[var(--ink)]">Home</Link><span>/</span><span className="text-[var(--ink)]">Hoodies</span></nav>
          <h1 className="text-4xl md:text-5xl text-[var(--ink)]" style={{ fontFamily: 'var(--font-heading)' }}>Hoodies</h1>
          <p className="mt-4 text-[var(--stone)]">{allHoodies.length} figures available</p>
        </div>
      </section>
